...
export default function Home() {
    ...
    const { recipes, filterByDiet, orders, recipesPerPage, page } = useSelector(
        (state) => state
    );
    const dietsFilter = (recipe) =>
        recipe.diets?.some((d) => d.name === filterByDiet || !filterByDiet);
    const pagination = (_, i) =>
        recipesPerPage * page <= i && i < recipesPerPage * (page + 1);

    useEffect(() => {
        dispatch(getRecipes());
    }, [dispatch]);
    const filteredAndOrderedRecipes = recipes
        ?.sort(orderBy[orders])
        .filter(dietsFilter);
    return (
        <>
